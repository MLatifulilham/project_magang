...
export default defineEventHandler(async (event) => {
    try {
        ...
        const { asset_id, tujuan_penggunaan, lokasi_tujuan, tgl_pinjam, tgl_rencana_kembali } = body

        if (!asset_id || !tujuan_penggunaan || !lokasi_tujuan || !tgl_pinjam || !tgl_rencana_kembali) {
            throw createError({
                statusCode: 400,
                statusMessage:
                    'asset_id, tujuan_penggunaan, lokasi_tujuan, tgl_pinjam, dan tgl_rencana_kembali wajib diisi',
            })
        }
      ...
      `,
            [asset_id, user.id, tujuan_penggunaan, lokasi_tujuan, tgl_pinjam, tgl_rencana_kembali],
        )
